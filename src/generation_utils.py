...
class GenerationMixin:
    ...
    def generate(
        ...
        begin_suppress_tokens: Optional[List[int]] = None,
        forced_decoder_ids: Optional[List[List[int]]] = None,
        block_context: Optional[bool] = None,
        prefix_len: Optional[int] = None, # for context-aware contrastive search decoding
        source_len: Optional[int] = None, # for context-aware contrastive search decoding
        source_penalty_beta: Optional[float] = None, # for context-aware contrastive search decoding
        **model_kwargs,
        ...
        is_context_aware_contrastive_search_gen_mode = (
            top_k is not None
            and top_k > 1
        ...
        if is_greedy_gen_mode:
        ...
        elif is_context_aware_contrastive_search_gen_mode:

            # print('context-aware contrastive search')
            ...
            return self.context_aware_contrastive_search(
                input_ids,
                top_k=top_k,
                ...
                source_penalty_beta=source_penalty_beta,
                **model_kwargs,
            )
    ...
    @torch.no_grad()
    def context_aware_contrastive_search(
        self,
        input_ids: torch.LongTensor,
        ...
        source_penalty_beta: Optional[float] = None,
        **model_kwargs,
        ...
        while True:
            ...
            # model confidence
            selected_idx = _context_aware_ranking_fast(
                context_hidden,
                next_hidden,
                top_k_probs,
                penalty_alpha,
...
def _context_aware_ranking_fast(
    context_hidden: torch.FloatTensor,
    next_hidden: torch.FloatTensor,
    next_top_k_probs: torch.FloatTensor,
    alpha: float,
    beam_width: int,
    block_context: Optional[bool],
    prefix_len: int,
    source_len: Optional[int],
    source_penalty_beta: Optional[float],
) -> torch.FloatTensor:
    """
    Reranks the top_k candidates based on a degeneration penalty (cosine similarity with previous tokens), as described
    in the paper "A Contrastive Framework for Neural Text Generation". Returns the index of the best candidate for each
    ...
    if source_len is not None:
        assert source_penalty_beta is not None
        source_penalty, _ = torch.max(cosine_matrix[:, prefix_len-source_len: prefix_len], dim=-1)  # [B*K]
        if block_context: # prevent computing degeneration penalty from the pre-given context
            cosine_matrix[:, :prefix_len] = -1
        degeneration_penalty, _ = torch.max(cosine_matrix, dim=-1)  # [B*K]
        next_top_k_probs = next_top_k_probs.view(-1)  # [B*K]
        # degeneration_penalty = (degeneration_penalty + 1) / 2 # normalize [-1, 1] to [0, 1]
        # source_penalty = (source_penalty + 1) / 2 # normalize [-1, 1] to [0, 1]
        contrastive_score = (1.0 - alpha - source_penalty_beta) * next_top_k_probs \
            - alpha * degeneration_penalty \
            + source_penalty_beta * source_penalty
    else:
        assert block_context
        cosine_matrix[:, :prefix_len] = -1 # prevent computing degeneration penalty from the pre-given context
        degeneration_penalty, _ = torch.max(cosine_matrix, dim=-1)  # [B*K]
        # degeneration_penalty = (degeneration_penalty + 1) / 2 # normalize [-1, 1] to [0, 1]
        next_top_k_probs = next_top_k_probs.view(-1)  # [B*K]
        contrastive_score = (1.0 - alpha) * next_top_k_probs - alpha * degeneration_penalty
